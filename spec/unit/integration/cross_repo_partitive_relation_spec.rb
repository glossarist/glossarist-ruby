...
# Cross-repo integration: verifies that a PartitiveHyperedge YAML
...
RSpec.describe "Cross-repo per-file relation integration" do
  ...
  it "loads every relation file via RelationLoader" do
    ...
    relations.each_value do |list|
      list.each do |rel|
        expect(rel).to be_a(Glossarist::V3::AbstractHyperedge)
        expect(rel.members.length).to be >= 2
      end
    end
  end

  it "loads the VIM measurement-result-composition PartitiveHyperedge" do
  ...
  it "every loaded PartitiveHyperedge passes validate!" do
    relations = Glossarist::V3::RelationLoader.load_all(concept_model_relations_dir)
    relations.values.flatten.each do |rel|
      next unless rel.is_a?(Glossarist::V3::PartitiveHyperedge)
