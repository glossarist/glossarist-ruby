...
RSpec.describe Glossarist::Commands::ValidateIdLinkages do
  ...
  context "when concept file is valid" do
    let(:new_glossary_path) do
      File.expand_path("../fixtures/validate_id_linkages/new", __dir__)
    end

    let(:expected_report) do
      File.expand_path(
        "../fixtures/validate_id_linkages/valid_report.txt",
        __dir__,
      )
    end
  ...
  context "when concept file has invalid ID linkages" do
    let(:new_glossary_path) do
      File.expand_path("../fixtures/validate_id_linkages/new_invalid", __dir__)
    end

    let(:expected_report) do
      File.expand_path(
        "../fixtures/validate_id_linkages/invalid_report.txt",
        __dir__,
      )
    end
    ...
    it "reports missing localized concepts" do
      test_subject
      output = File.read(test_output_path)
      expected_output = File.read(expected_report)
      expect(
        output.include?(
          ...
          "action_schema.action_resource-eng",
        ),
      ).to be true
      expect(output).to eq(expected_output)
    end
